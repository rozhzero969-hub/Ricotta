...
/* ============ Settings RPCs ============
   ...
   These are the ONLY server-side functions this app has (there is no
   separate API/Edge Function backend) -- every one of them is called
   directly as a PostgREST RPC via rpcCall() below. */
...
/* Returns 'admin', 'user', or null. Backed by the app_verify_pin(p_pin)
   Postgres function, which checks the pin against the 'settings' row and
   never exposes the stored PINs themselves. */
