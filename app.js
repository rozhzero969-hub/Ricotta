/* Ricotta Orders -- app state, rendering and event handlers.
   ...
   showConfirm, showAlert and showPrompt (modals.js). Load this file last. */
/* ============ State ============ */
let state = {
  lang: 'en',
  ...
  editingItem: null,
  devices: [],           // [{id, nickname, role, lastLogin}], synced via Supabase
  deviceId: null,         // this device's own id, generated once and kept locally
  editingDeviceId: null   // device currently being renamed in Settings, or null
};
...
/* Called right after a successful PIN login. Updates (or creates) this
   device's entry in the shared devices list with its role and the current
   time, then -- only the first time this device has ever logged in, i.e.
   it doesn't have a nickname yet -- asks for a nickname. Runs in the
   background after render() so it never delays the login transition. */
async function recordDeviceLogin(role){
  const id = state.deviceId;
  const now = new Date().toISOString();
  let entry = state.devices.find(d=>d.id===id);
  if(entry){ entry.role = role; entry.lastLogin = now; }
  else { entry = {id, nickname:'', role, lastLogin: now}; state.devices.push(entry); }
  await sset('devices', state.devices, true);
  if(!entry.nickname){
...
/* ============ Boot ============ */
async function boot(){
  const [suppliers, items, units, history, devices, lang] = await Promise.all([
    sget('suppliers', true), sget('items', true), sget('units', true),
    sget('orderHistory', true), sget('devices', true), sget('lang', false)
  ]);
...
function attachCommonEvents(){
  ...
  document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>{
    state.view=b.dataset.view; state.editingSupplier=null; state.editingItem=null; render();
  });
}
...
function renderSuppliers(){
  const form = `
    <div class="form-card">
      <div class="field"><label>${t('name')}</label><input id="supName" value="${esc(state.editingSupplier?.name||'')}"></div>
      <div class="field"><label>${t('phone')}</label><input id="supPhone" placeholder="07xxxxxxxxx" value="${esc(state.editingSupplier?.phone||'')}"></div>
...
function renderItemsAdmin(){
  const unitOptions = state.units.map(u=>`<option value="${esc(u.id)}" ${state.editingItem?.unit===u.id?'selected':''}>${esc(state.lang==='ku'?(u.ku||u.en):u.en)}</option>`).join('');
  // While adding a fresh item (not editing an existing one), the last
  // supplier picked stays selected so bulk-adding items for one supplier
  // doesn't require reselecting it every time.
  const currentSupplierId = state.editingItem ? state.editingItem.supplierId : state.itemFormSupplierId;
  ...
  const lockedSupplier = !state.editingItem && state.itemFormSupplierId ? state.suppliers.find(s=>s.id===state.itemFormSupplierId) : null;
  const form = `
    <div class="form-card">
      <div class="field"><label>${t('name')}</label><input id="itemName" value="${esc(state.editingItem?.name||'')}"></div>
      <div class="field"><label>${t('unit')}</label><select id="itemUnit">${unitOptions}</select></div>
      <div class="field">
        <label>${t('supplier')}</label>
        <select id="itemSupplier">${supOptions}</select>
        ${lockedSupplier ? `<div class="field-hint">${t('supplierStaysSelected')(esc(lockedSupplier.name))}</div>` : ''}
      </div>
      <div class="form-actions">
...
function attachItemEvents(){
  ...
  if(supSelect) supSelect.onchange = ()=>{
    if(!state.editingItem) state.itemFormSupplierId = supSelect.value || null;
  };
...
/* ============ Admin: Devices ============ */
/* Shown tucked inside the unlocked Cloud setup card in Settings, since
   this is admin-only, low-traffic info -- not worth its own nav tab. */
function renderDevices(){
  if(!state.devices.length) return `<div class="empty">${t('noDevicesYet')}</div>`;
  const editing = state.devices.find(d=>d.id===state.editingDeviceId);
  const renameForm = editing ? `
    <div class="form-card">
      <div class="field"><label>${t('renameDevice')}</label><input id="deviceNicknameInput" value="${esc(editing.nickname)}"></div>
      <div class="form-actions">
        <button class="btn btn-primary" id="deviceNicknameSaveBtn">${t('save')}</button>
        <button class="btn btn-ghost" id="deviceNicknameCancelBtn">${t('cancel')}</button>
      </div>
    </div>` : '';
  const sorted = [...state.devices].sort((a,b)=> new Date(b.lastLogin) - new Date(a.lastLogin));
  const rows = sorted.map(d=>{
    const isThis = d.id === state.deviceId;
    const roleLabel = d.role === 'admin' ? t('deviceRoleAdmin') : t('deviceRoleStaff');
    const when = new Date(d.lastLogin).toLocaleString(state.lang==='ku'?'en-GB':'en-US',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
    return `<div class="list-row">
      <div><div class="name">${esc(d.nickname) || t('unnamedDevice')}${isThis?` <span style="color:var(--basil);font-weight:700;font-size:11px;">· ${t('thisDevice')}</span>`:''}</div>
      <div class="meta">${roleLabel} · ${when}</div></div>
      <div class="row-actions"><button class="icon-btn" data-editdevice="${d.id}">${ICON_EDIT}</button></div>
    </div>`;
  }).join('');
  return `${renameForm}${rows}`;
}
function attachDeviceEvents(){
  document.querySelectorAll('[data-editdevice]').forEach(b=>b.onclick=()=>{
    state.editingDeviceId = b.dataset.editdevice; render();
  });
  const saveBtn = document.getElementById('deviceNicknameSaveBtn');
  if(saveBtn) saveBtn.onclick = async ()=>{
    const val = document.getElementById('deviceNicknameInput').value.trim();
    const d = state.devices.find(x=>x.id===state.editingDeviceId);
    if(d) d.nickname = val;
    await sset('devices', state.devices, true);
    state.editingDeviceId = null; render();
  };
  const cancelBtn = document.getElementById('deviceNicknameCancelBtn');
  if(cancelBtn) cancelBtn.onclick = ()=>{ state.editingDeviceId = null; render(); };
}
...
function renderSettings(){
  ...
  if(!state.cloudUnlocked){
    ...
    cloudCard = `
      ...
      <div class="section-title">${t('devicesTitle')}</div>
      ${renderDevices()}`;
  }
...
function attachSettingsEvents(){
  ...
  if(state.cloudUnlocked) attachDeviceEvents();
}
