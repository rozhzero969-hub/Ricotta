...
function attachCommonEvents(){
  ...
  if(lo) lo.onclick = ()=>{ state.role=null; state.pinBuffer=''; state.view='order'; state.cloudUnlocked=false; lset('session', null); render(); };
  ...
  document.querySelectorAll('[data-key]').forEach(b=>b.onclick=()=>{
    ...
    if(state.pinBuffer.length===USER_PIN_LEN && !state.pinExpanded){
      if(state.pinBuffer===state.settings.userPin){
        state.role='user'; state.pinBuffer=''; lset('session', 'user'); render(); return;
      }
    ...
    if(state.pinBuffer.length===ADMIN_PIN_LEN){
      if(state.pinBuffer===state.settings.adminPin){ state.role='admin'; state.pinBuffer=''; state.pinExpanded=false; lset('session', 'admin'); render(); return; }
