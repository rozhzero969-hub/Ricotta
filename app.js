...
function renderDevices(){
  ...
  const loggedInCount = state.devices.filter(isLoggedIn).length;
  ...
  const cards = sorted.map(d=>{
    ...
    const group = st === 'out' ? 'out' : 'in';
    let heading = '';
    if(group !== lastGroup){
      lastGroup = group;
      const n = state.devices.filter(x=>(deviceStatus(x)==='out') === (group==='out')).length;
      heading = `<div class="section-title dev-group">${group==='out' ? t('statusLoggedOut') : t('statusLoggedIn')} (${n})</div>`;
    }
    ...
    const recent = logins.length > 1
      ...
      : '';
    return `${heading}<div class="dev-card">
      <div class="dev-top">
