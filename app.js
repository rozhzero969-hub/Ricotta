...
/* ============ Boot ============ */
/* All catalog/history data lives in the shared 'app_data' table (read via
   sget, written via sset -- see storage.js), and login is a stateless PIN
   check (appVerifyPin) against the same table's 'settings' row. There is
   no separate login-session/token backend, so the only thing that persists
   a signed-in role across a page reload is the local 'session' flag set
   below. */
async function boot(){
