/* Ricotta Orders -- app state, rendering and event handlers.
   ...
   (modals.js), hardReload (update-check.js, only called at runtime). Load this
   file after modals.js. */
/* ============ State ============ */
let state = {
  lang: 'en',
...
function updateDevice(id, mutate){
  return mutateDevices(list=>{
    ...
    if(!entry){
      entry = {id, nickname:'', role:'user', lastLogin:null, lastSeen:null, loggedIn:false, logins:[]};
      list.push(entry);
    }
    mutate(entry);
  });
...
async function recordDeviceLogin(role){
  ...
  if(entry && !entry.nickname){
    ...
    if(name) await updateDevice(state.deviceId, d=>{ d.nickname = name; });
  }
}
...
function sendDeviceCommand(type, ids){
  ...
  return mutateDevices(list=>{
    list.forEach(d=>{ if(ids.includes(d.id)) d.command = {id:newCommandId(), type, ts}; });
  });
}
...
function renderDevices(){
  const head = `
    <div class="action-row">
      <div class="section-title" style="flex:1;margin:0;">${t('devicesTitle')} (${state.devices.length})</div>
      <button class="btn btn-ghost" id="devRefreshBtn">${ICON_REFRESH} ${t('refresh')}</button>
    </div>`;
  if(!state.devices.length) return `${head}<div class="empty">${t('noDevicesYet')}</div>`;
  ...
  const loggedInCount = state.devices.filter(d=>d.loggedIn).length;
  const activeCount = state.devices.filter(d=>deviceStatus(d)==='active').length;
  ...
  const bulk = others.length ? `<div class="dev-bulk">
      <button class="btn btn-primary" id="devRefreshAllBtn">${ICON_REFRESH} ${t('refreshAllDevices')}</button>
      <button class="btn btn-danger" id="devLogoutAllBtn" ${others.some(d=>d.loggedIn)?'':'disabled'}>${t('logoutAllOthers')}</button>
    </div>` : '';
  ...
  const cards = sorted.map(d=>{
    ...
    return `<div class="dev-card">
      ...
      ${isThis ? '' : `<div class="dev-actions">
        ...
        ${d.loggedIn ? `<button class="btn btn-danger" data-devlogout="${esc(d.id)}">${t('logoutDevice')}</button>` : ''}
      </div>`}
    </div>`;
  }).join('');
...
function attachDeviceEvents(){
  ...
  if(allRefresh) allRefresh.onclick = async()=>{
    const ids = otherDevices().map(d=>d.id);
  ...
  if(allLogout) allLogout.onclick = async()=>{
    const ids = otherDevices().filter(d=>d.loggedIn).map(d=>d.id);
    if(!ids.length) return;
