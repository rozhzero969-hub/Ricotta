...
async function authenticate(req: Request): Promise<Session | null> {
  ...
  if (!raw) return null;
  const { data } = await db.schema("app_private").from("sessions")
  ...
  await db.schema("app_private").from("sessions").update({ last_seen_at: new Date().toISOString() }).eq("id", data.id);
...
async function audit(session: Session, req: Request, action: string, type?: string, name?: string, payload: Record<string, unknown> = {}) {
  await db.schema("app_private").from("audit_events").insert({
...
async function bootstrap() {
  const [suppliers, items, units, orders, devices, activity, reminder] = await Promise.all([
    db.schema("app_private").from("suppliers").select("id,name,phone,reminder").order("name"),
    db.schema("app_private").from("items").select("id,name,unit_id,supplier_id").order("name"),
    db.schema("app_private").from("units").select("id,en,ku"),
    db.schema("app_private").from("orders").select("id,status,created_at,sent_at").eq("status", "sent").order("sent_at"),
    db.schema("app_private").from("devices").select("id,nickname,role,logged_in,last_login,last_seen,command,handled_command"),
    db.schema("app_private").from("audit_events").select("id,occurred_at,actor_role,device_id,action,entity_type,entity_name,payload").order("occurred_at", { ascending: false }).limit(1000),
    db.schema("app_private").from("reminder_settings").select("enabled,remind_time").eq("id", true).maybeSingle(),
  ]);
  const orderIds = (orders.data ?? []).map(o => o.id);
  const { data: lines } = orderIds.length ? await db.schema("app_private").from("order_lines")
...
Deno.serve(async req => {
  ...
  if (req.method === "POST" && path === "login") {
    ...
    const { count } = await db.schema("app_private").from("login_attempts").select("id", { count:"exact", head:true })
    ...
    await db.schema("app_private").from("login_attempts").insert({ fingerprint_hash:fingerprint, succeeded:success });
    ...
    await db.schema("app_private").from("sessions").insert({ token_hash:await hash(token), role, device_id:deviceId, expires_at:expiresAt });
    if (deviceId) await db.schema("app_private").from("devices").upsert({ id:deviceId, role, logged_in:true, last_login:new Date().toISOString(), last_seen:new Date().toISOString() });
  ...
  if (req.method === "POST" && path === "logout") {
    await db.schema("app_private").from("sessions").update({ revoked_at:new Date().toISOString() }).eq("id", session.id);
    if (session.deviceId) await db.schema("app_private").from("devices").update({ logged_in:false }).eq("id", session.deviceId);
    return json({ ok:true });
  }
  ...
  if (stateMatch) {
    ...
    if (req.method === "PUT" && Array.isArray(payload.value)) {
      ...
      if (key === "suppliers") {
        await db.schema("app_private").from("suppliers").delete().not("id", "in", `(${value.map((x:any)=>`\"${cleanText(x.id,120).replaceAll('"','')}\"`).join(",") || "\"__none__\""})`);
        ...
        if(rows.length) await db.schema("app_private").from("suppliers").upsert(rows);
      } else if (key === "units") {
        ...
        if(rows.length) await db.schema("app_private").from("units").upsert(rows);
      } else if (key === "items") {
        ...
        if(rows.length) await db.schema("app_private").from("items").upsert(rows);
      } else if (key === "devices") {
        ...
        if(rows.length) await db.schema("app_private").from("devices").upsert(rows);
        ...
        if(rows.length) await db.schema("app_private").from("audit_events").upsert(rows);
        ...
        if(rows.length) await db.schema("app_private").from("orders").upsert(rows);
        ...
        if(lines.length) await db.schema("app_private").from("order_lines").upsert(lines,{onConflict:"order_id,supplier_id,item_id"});
      }
  ...
  if (match) {
    ...
    if (req.method === "DELETE" && entityId) { await db.schema("app_private").from(table).delete().eq("id", entityId); await audit(session,req,"delete",entity,entityId); return json({ok:true}); }
    if (req.method === "POST") {
      ...
      const { error } = await db.schema("app_private").from(table).upsert(record); if(error) return json({error:"invalid_input"},400);
