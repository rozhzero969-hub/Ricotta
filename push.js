...
async function savePushSubscription(sub){
  ...
  const ok = await rpcCall('push_save_subscription', {
    p_endpoint:j.endpoint, p_p256dh:j.keys.p256dh, p_auth:j.keys.auth, p_device_id:deviceId
  });
  return ok === true;
}
...
/* "New update" notification to every device that has notifications on. The
   admin writes the What's new text themselves each time it's sent, so the
   message always matches what's actually being announced -- it isn't tied
   to whatever CHANGELOG_EN/KU happens to be sitting in config.js. */
async function sendUpdateNotification(){
  await showFormModal({
    title: t('sendUpdateNotif'),
    bodyHtml: `<div class="field">
        <label>${t('updateMessageLabel')}</label>
        <textarea id="mfUpdateMsg" rows="4" data-clear="1" placeholder="${esc(t('updateMessagePlaceholder'))}"></textarea>
      </div>`,
    okLabel: t('notifSend'),
    onOpen: (box)=>{ const ta = box.querySelector('#mfUpdateMsg'); if(ta) ta.focus(); },
    onSubmit: async ()=>{
      const msg = document.getElementById('mfUpdateMsg').value.trim();
      if(!msg) return { error: t('updateMessageRequired') };
      const res = await callSendPush('update', { title:'Ricotta Orders \u2014 update', body: msg });
